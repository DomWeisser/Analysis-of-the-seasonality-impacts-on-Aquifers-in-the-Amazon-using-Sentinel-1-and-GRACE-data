...
Monthly gravity anomalies were obtained from NASA's Jet Propulsion Laboratory for the year 2019. These data are gridded at 0.5° × 0.5° spatial resolution and provided in NetCDF format. The mascon solution offers improved spatial fidelity and error suppression compared to spherical harmonics, making it more suitable for hydrological applications in data-sparse regions like the Amazon.
- GLDAS Reanalysis data
- Sentinel-1 
- Alter do CHao Aquifer Shapefile
...
### In-situ Well Data
- Well data was filtered to include only stations with complete monthly records for 2019
- Spatial distribution analysis was performed to ensure adequate coverage
- Well coordinates were converted to a consistent geographic coordinate system
- Quality control was applied to remove outliers and erroneous measurements
### GRACE-FO 
...
### Sentinel-1 
- Sentinel-1 GRD data was processed to extract VH polarization backscatter
- Precise geolocation was applied using the metadata in the product annotation files
- Data was radiometrically calibrated and terrain corrected
- Speckle filtering was applied using a 5×5 Lee filter
- Images were clipped to the ACA boundary and resampled to 0.01° resolution
- Monthly backscatter statistics were calculated and temporal analysis performed
